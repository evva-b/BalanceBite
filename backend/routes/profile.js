        ...
        res.json({
            ...
            data: {
                ...
                stats: {
                    ...
                    averageProteins: result.rows.length > 0 ? result.rows.reduce((a, b) => a + Number(b.total_proteins), 0) / result.rows.length : 0,
                    averageFats: result.rows.length > 0 ? result.rows.reduce((a, b) => a + Number(b.total_fats), 0) / result.rows.length : 0,
                    averageCarbs: result.rows.length > 0 ? result.rows.reduce((a, b) => a + Number(b.total_carbs), 0) / result.rows.length : 0,
                    averageCalories: result.rows.length > 0 ? result.rows.reduce((a, b) => a + Number(b.total_calories), 0) / result.rows.length : 0
                }
            }
        });
    } catch (err) {
        ...
        if (tableCheck.rows[0].exists) {
         ...
         ORDER BY measurement_date ASC`,
            ...
            if (measurements.length > 0) {
                ...
                stats.averageWeight = parseFloat((weights.reduce((a, b) => a + b, 0) / weights.length).toFixed(2));
