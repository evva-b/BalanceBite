...
export const ProfilePage: React.FC = () => {
  ...
  const loadProfileData = async () => {
    ...
    if (data) {
      ...
      // Обновляем метрики из ответа бэкенда
      setMetrics({
        bmi: data.bmi || null,
        bmiCategory: data.bmi_category || '', // Убедитесь, что имена полей совпадают с бэкендом
        idealWeight: data.ideal_weight || '',
        bmr: data.bmr?.toString() || '',
        calories: data.daily_calorie_norm || null,
      ...
      if (data.favoriteProducts) {
        setProducts((prev) => ({ ...prev, favorite: data.favoriteProducts }));
      }
      if (data.dislikedProducts) {
        setProducts((prev) => ({ ...prev, notFavorite: data.dislikedProducts }));
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      setSaveSuccess(true);
      // Перезагружаем данные: бэкенд при сохранении пересчитал калории,
      // и теперь мы их получим в loadProfileData
  ...
  const handleGoalChange = (goal: string) => {
    ...
    // Убрали setTimeout с updateCalories
  };
                    ...
                    <div className="mt-2 ml-6 space-y-2">
                      <div className="flex flex-wrap gap-2 mb-2">
                        {products.favorite.map((product) => (
                          <div
                            key={product.id}
                            className="flex items-center gap-1 bg-[#F0F0F0] rounded-full px-3 py-1"
                          >
