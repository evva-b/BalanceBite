...
export const AnalyticsPage: React.FC = () => {
      ...
      if (weightData.success) {
        ...
        if (weightData.data.goal) {
          const currentWeightNum =
            typeof weightData.data.goal.current_weight === 'string'
  ...
  const getTargetWeight = (): number => {
    ...
    const weightNum =
      typeof currentWeight === 'number' ? currentWeight : parseFloat(String(currentWeight));
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <div className="grid grid-cols-7 gap-1">
                {daysInMonth.map((day, idx) => {
                  ...
                  const isStartDate =
                    selectedStartDate && formatDate(day) === formatDate(selectedStartDate);
                  const isEndDate =
                    selectedEndDate && formatDate(day) === formatDate(selectedEndDate);
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center text-[14px]">
                        <div className="bg-gray-50 rounded-lg p-2">
                          ...
                          <span className="font-bold text-[#58B079]">
                            {startWeight !== null && !isNaN(startWeight)
                        ...
                        <div className="bg-gray-50 rounded-lg p-2">
                          ...
                          <span className="font-bold text-[#58B079]">
                            {endWeight !== null && !isNaN(endWeight) ? endWeight.toFixed(1) : '—'}{' '}
                            кг
                          </span>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-2">
                          ...
                          <span
                            className={`font-bold ${weightChange && weightChange < 0 ? 'text-green-500' : weightChange && weightChange > 0 ? 'text-red-500' : 'text-[#58B079]'}`}
                          >
                            {weightChange !== null && !isNaN(weightChange) && weightChange > 0
                              ? '+'
                              : ''}
                            {weightChange?.toFixed(1) || '0'} кг
                          </span>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-2">
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      ...
                      >
                        ...
                        <YAxis
                          ...
                          label={{ value: 'г', angle: -90, position: 'insideLeft', fontSize: 11 }}
                        />
                        <YAxis
                          yAxisId="right"
                          orientation="right"
                ...
                <div className="bg-white rounded-2xl p-4 shadow-sm">
                  ...
                  {hasNutritionData ? (
                    ...
                    <div className="text-center text-[12px] text-orange-500 mb-3">
                      Нет данных о питании за выбранный период. Добавьте приемы пищи в Дневник
                      питания.
                    </div>
                  )}
